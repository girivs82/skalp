...
impl MobileSoC {
    ...
    pending_irq = (wake_sources != 0) || (cpu_to_irq != 0);
}
```
...
### UPF Generation (Planned)

> **Status: not yet implemented.** There is currently no `--emit-upf` flag; the
> compiler emits the synthesis attributes shown above but does not generate UPF
> files. The design intent is for Skalp to eventually emit standard UPF for EDA
> tools, along these lines:

```tcl
# Planned output (illustrative — not produced by the current compiler)
