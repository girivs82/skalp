...
## Sequential NCL (Boundary Mode)

### Overview

Boundary NCL entities can contain **sequential state** — registers that update
on each DATA phase, with no clock signal. The `on()` block with no arguments
expresses this: "update state when all inputs have valid DATA."

The completion detection of all inputs acts as a local clock for internal
registers. This is synthesized as standard DFFs clocked by the completion
signal, wrapped in the NCL boundary (dual-rail encode/decode at I/O).

### The `on()` Pattern

In an `async entity`, `on()` with no arguments means "on each DATA phase":

```skalp
async entity Counter {
    in enable: bit
    in increment: bit[8]
    out count: bit[8]
}
...
impl Counter {
    ...
    on() {
        count_reg = if enable { count_reg + increment } else { count_reg }
    }
...
**Semantics:**
- `on()` fires when ALL inputs transition from NULL to valid DATA
- Assignments inside `on()` create registers (same as `on(clk.rise)`)
- Between DATA phases (during NULL), registers hold their value
- Initial values (`= 0`) set the power-on state

**Comparison with clocked design:**

| Clocked | NCL (boundary) |
|---------|----------------|
| `entity Foo { in clk: clock, in rst: reset, ... }` | `async entity Foo { ... }` |
| `on(clk.rise) { if rst { x = 0 } else { x = expr } }` | `on() { x = expr }` |
| Clock tree distributes timing | Completion detection is local |
| Reset tree for initialization | FPGA init / ASIC POR (automatic) |

### Initialization (No Reset Needed)

NCL sequential state does not require explicit reset signals.

**FPGA targets:**
- `signal x: bit[8] = 0` → FPGA bitstream INIT attribute on the DFFs
- DFFs hold their initialized value until first DATA phase completes
- No reset port, no reset routing, no reset timing closure

**ASIC targets:**
- Power-on reset (POR) is generated by the synthesizer automatically
- POR is a one-shot async clear — much simpler than synchronous reset
- No timing relationship to maintain (NCL is async)
- The `= value` syntax sets the POR target state

**Why this works:** NCL signals start in NULL state at power-on. Completion
detection doesn't fire until valid DATA arrives. So registers sit at their
initial value with no risk of spurious updates.

### Synthesis Flow

For boundary NCL with `on()` blocks, the synthesizer:

1. **Input boundary**: NclDecode (dual-rail → single-rail) for each input
2. **Completion detection**: AND-tree of all input rail XORs → `__ncl_complete`
3. **Internal registers**: Standard DFFs clocked by `__ncl_complete`
4. **Combinational logic**: Single-rail, same as synchronous designs
5. **Output boundary**: NclEncode (single-rail → dual-rail) for each output

The internal register clock is NOT a global clock — it's a local signal
derived from the specific inputs of this entity. Different entities have
independent completion detection, enabling true asynchronous composition.

### Examples

#### Shift Register (no clock, no reset)

```skalp
async entity ShiftReg4 {
    in data_in: bit[8]
    out data_out: bit[8]
}
...
Each DATA/NULL cycle shifts one position. No clock distribution needed.

#### Traffic Light FSM

```skalp
async entity TrafficLight {
    in trigger: bit
    out led_red: bit
    out led_green: bit
    out led_yellow: bit
...
impl TrafficLight {
    signal state: bit[2] = 0    // 0=RED, 1=GREEN, 2=YELLOW

    on() {
        state = if state == 2'b00 {
...
#### Instruction Fetch Unit (GPU example)

```skalp
async entity InsnFetch {
    in start: bit
    in branch_valid: bit
    in branch_target: bit[32]
    out insn: bit[32]
    out insn_pc: bit[32]
    out insn_valid: bit
}
...
impl InsnFetch {
    #[memory(depth = 256, width = 32, style = block)]
    signal icache: [bit[32]; 256]

    signal pc: bit[32] = 32              // skip 32-byte header
    signal cache_valid: bit[256] = 0
    ...
    // Combinational read from cache
    let idx = pc[9:2]
    insn = icache[idx]
    insn_pc = pc
    insn_valid = cache_valid[idx]
}
```
