...
use skalp_lir::{
    apply_boundary_ncl_to_hierarchy, get_stdlib_library,
    lower_mir_hierarchical_for_optimize_first, synthesize_hierarchical,
};
...
fn test_bit_mul_resolves_through_stdlib() {
    ...
    eprintln!("=== Module HIRs: {} modules loaded ===", context.module_hirs.len());
    for (path, hir) in &context.module_hirs {
        ...
        for impl_ in &hir.trait_implementations {
            ...
            for method in &impl_.method_implementations {
                eprintln!("        fn {} ({} body stmts)", method.name, method.body.len());
            }
    ...
    // Also check main HIR
    eprintln!("=== Main HIR ===");
    eprintln!("  Trait impls: {}", context.main_hir.trait_implementations.len());
    ...
    let netlist = synthesize_hierarchical(
        &hier_lir,
        &library,
