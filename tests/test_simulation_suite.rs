...
mod simulation_suite {
    ...
    async fn test_counter_increments() {
        let counter_source = fs::read_to_string("examples/counter.sk")
            .expect("Failed to read counter.sk");
        ...
        for i in 0..20 {
            sim.set_input("clk", vec![((i + 4) % 2) as u8]).await.unwrap();
            sim.step_simulation().await.unwrap();

            if i % 2 == 1 {  // After rising edge
    ...
    async fn test_alu_operations() {
        let alu_source = fs::read_to_string("examples/alu.sk")
            .expect("Failed to read alu.sk");
        ...
        // Test AND operation
        sim.set_input("a", vec![0xFF, 0x00, 0xFF, 0x00]).await.unwrap();
        sim.set_input("b", vec![0x0F, 0xF0, 0x0F, 0xF0]).await.unwrap();
    ...
    async fn test_fifo_operations() {
        let fifo_source = fs::read_to_string("examples/fifo.sk")
            .expect("Failed to read fifo.sk");
    ...
    async fn test_simulation_state_consistency() {
        ...
        assert_eq!(states2[0..initial_len], states1[..],
