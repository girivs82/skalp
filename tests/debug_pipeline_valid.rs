...
mod debug_pipeline_valid_tests {
    ...
    async fn test_pipeline_valid_only() {
        ...
        let mut testbench = Testbench::new(config).await.expect("Failed to create testbench");
        testbench.load_module(&sir).await.expect("Failed to load module");
        ...
        testbench.add_test_vector(TestVectorBuilder::new(34)  // GPU cycle 17
            .with_input("rst", vec![0])
