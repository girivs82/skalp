...
/// Insert power infrastructure cells into LIR
pub fn insert_power_infrastructure(
    ...
    for req in &analysis.level_shifters_needed {
        let new_prim_id = PrimitiveId(next_prim_id);
        next_prim_id += 1;

        let new_net_id = NetId(next_net_id);
        next_net_id += 1;

        // Create level shifter primitive
        let level_shifter = Primitive::new_level_shifter(
            new_prim_id,
            format!("power_infra/lvlshift_{}", req.net_id.0),
            req.from_voltage_mv,
            req.to_voltage_mv,
            req.net_id,
            new_net_id,
        ...
        // Create new net for level shifter output
        let new_net = LirNet::new(new_net_id, format!("lvlshift_out_{}", req.net_id.0));
    ...
    for req in &analysis.isolation_cells_needed {
        ...
        // For now, we'd need an isolation enable net - simplified here
        let iso_en_net = NetId(next_net_id);
        next_net_id += 1;

        // Create isolation cell primitive
        let isolation_cell = Primitive::new_isolation_cell(
            new_prim_id,
            format!("power_infra/iso_{}", req.net_id.0),
            req.clamp_value,
            true, // active high
            req.net_id,
            iso_en_net,
        ...
        let new_net = LirNet::new(new_net_id, format!("iso_out_{}", req.net_id.0));
