...
fn test_spi_dff_sir_structure() {
    let hir = parse_and_build_hir_from_file(
        Path::new(env!("CARGO_MANIFEST_DIR")).join("../../examples/spi_master.sk").as_path()
    ).expect("parse");
    ...
    for net in &netlist.nets {
        if net.name.contains("shift_reg") || net.name.contains("_t21") {
            println!("Net {}: name={} is_output={} driver={:?} alias_of={:?}",
    ...
    for (i, node) in lir.nodes.iter().enumerate() {
        let out_name = lir.signals.get(node.output.0 as usize).map(|s| s.name.as_str()).unwrap_or("?");
        if out_name.contains("sclk") || format!("{:?}", node.op).contains("sclk") {
            let inp_names: Vec<&str> = node.inputs.iter()
                .map(|id| lir.signals.get(id.0 as usize).map(|s| s.name.as_str()).unwrap_or("?"))
                .collect();
            println!("  Node {}: op={:?} inputs={:?} output={} ({})",
    ...
    for signal in &sir.top_module.signals {
        if signal.name.contains("shift_reg") || signal.name.contains("_t21") {
            println!("Signal {}: name={} width={} type={:?} initial={:?}",
    ...
    for block in &sir.top_module.comb_blocks {
        for op in &block.operations {
            if let skalp_sim::sir::SirOperation::Primitive { id, ptype, inputs, outputs, path } = op {
                if path.contains("shift_reg") || path.contains("_t21") || path.contains("_t17")
                    || path.contains("_t16") || path.contains("_t20") || path.contains("sclk") {
                    println!("  Op {}: ptype={:?} inputs={:?} outputs={:?} path={}",
                        id.0, ptype, inputs, outputs, path);
                    for inp in inputs {
                        if let Some(sig) = sir.top_module.signals.iter().find(|s| s.id == *inp) {
                            println!("    INPUT signal {}: name={}", sig.id.0, sig.name);
    ...
    let print_key = |sim: &GateLevelSimulator, label: &str| {
        ...
        let get = |name: &str| -> String {
            snap.get(name).map(|b| format!("{}", b[0] as u8)).unwrap_or("?".into())
        };
        println!("  [{}] sclk_reg={} state={} reset={} start={}",
            label, get("top.sclk_reg"), get("top.state"), get("reset"), get("start"));
    };
    ...
    let snap = sim.snapshot_signals();
    for (name, bits) in &snap {
        if name.contains("shift_reg") || name.contains("_t21") || name.contains("_t17")
            || name.contains("_t16") || name.contains("_t20") || name.contains("state")
            || name.contains("sclk_reg") || name.contains("data_in") || name.contains("start")
            || name.contains("miso") || name.contains("data_out") {
            let val: u64 = bits.iter().enumerate()
    ...
    for (name, bits) in &snap {
        if name == "top.shift_reg[0]" {
            let val = bits[0];
            println!("\n*** shift_reg[0] = {} (expected 0, data_in[0]=0) ***", val);
