...
impl SsaConverter {
    ...
    fn update_lvalue_refs(&mut self, lvalue: &mut LValue) {
        stacker::maybe_grow(256 * 1024, 8 * 1024 * 1024, || {
            self.update_lvalue_refs_impl(lvalue)
        })
    }

    fn update_lvalue_refs_impl(&mut self, lvalue: &mut LValue) {
        match lvalue {
    ...
    fn update_expr_refs(&mut self, expr: &mut Expression) {
        stacker::maybe_grow(256 * 1024, 8 * 1024 * 1024, || {
            self.update_expr_refs_impl(expr)
        })
    }

    fn update_expr_refs_impl(&mut self, expr: &mut Expression) {
        match &mut expr.kind {
